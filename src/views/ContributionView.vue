      ...
      <div class="screen--inner">
        <div class="pads">
          <button
            ...
            :key="index"
            class="pads--item"
            @mousedown="padClick($event, pad, index)" @click="loadEmptyPreset"
          >
...
export default {
  ...
  created() {
    ...
    document.addEventListener("keydown", event => {
      ...
      if (event.keyCode === 49) {
        this.padClick(event, event.keyCode, 0);
      } else if (event.keyCode === 50) {
        this.padClick(event, event.keyCode, 1);
      } else if (event.keyCode === 81) {
        this.padClick(event, event.keyCode, 2);
      } else if (event.keyCode === 87) {
        this.padClick(event, event.keyCode, 3);
      } else if (event.keyCode === 65) {
        this.padClick(event, event.keyCode, 4);
      } else if (event.keyCode === 83) {
        this.padClick(event, event.keyCode, 5);
      } else if (event.keyCode === 90) {
        this.padClick(event, event.keyCode, 6);
      } else if (event.keyCode === 88) {
        this.padClick(event, event.keyCode, 7);
      }
  ...
  methods: {
    loadPreset(index) {
      ...
      this.$store.commit("setAudioParameterToPreset", {
        preset: presets[index].parameterValues
      });
      ...
      // Set bpm from preset
      // audio.setBpm(presets[index].bpm * 2); //temp switch off
      // Set bpm in store
      this.$store.commit("setPresetBpm", presets[index].bpm);
