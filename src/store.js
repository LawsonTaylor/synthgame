...
export default new Vuex.Store({
  ...
  mutations: {
    setStep(state, { note }) {
      // TEMP: experiment Bart: load recorded state on audioParameters per step.
      state.step = note;
    },
    loadRecordedState(state, { note, synth }) {
      // TEMP: experiment Bart: load recorded state on audioParameters per step.
      if (state.recordedParameters[note]) {
        state.audioParameters.oscillator1.detune =
          state.recordedParameters[note].oscillator1.detune;
      }
    },
  ...
  actions: {
    setRecordedState({ state, commit }) {
      // TEMP: experiment Bart: record recorded state on audioParameters per step.

      // Make sure to pass snapshot, or object will still be updated
      state.recordedParameters[state.step] = JSON.parse(
        JSON.stringify(state.audioParameters)
      ); //sets all, not just osc1 detune
      state.recordedParameters[state.step].recorded = true;
    },
